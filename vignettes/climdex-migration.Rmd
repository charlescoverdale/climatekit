...
Two rows above are acronym traps. `ck_cwd()` is **not** the CWD in that
table: it is the ET-SCI cold-wave duration index, and ETCCDI consecutive
wet days is `ck_wet_days()`. Likewise `ck_cooling_degree_days()` is not
ETCCDI CDD; consecutive dry days is `ck_dry_days()`. `ck_compute()`
...
2. **In-base bootstrap.** Percentile indices differ inside the base period
   unless you ask for the correction. Pass `bootstrap = TRUE` to
   `ck_tx10p()`, `ck_tn10p()`, `ck_tx90p()` or `ck_tn90p()` to apply the
   Zhang et al. (2005) leave-one-out resampling that `climdex.pcic` uses
   by default. It is off by default here because it is expensive.
